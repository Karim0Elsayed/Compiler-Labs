...
void setGrammar(CFG gmr , int n , char s , string fileLoc){
...
int main() {
	...
	char startSymbol;
	string str;
	cout<<"Grammar in text File Must be in Form S:prod1|....|prodn (S or Any Capetal Latter)"<<endl;
	cout<<"Grammar like this :"<<endl;
	cout<<"S:Abc|cBa \n" <<"B:Cvb|mXy"<<endl;
	cout<<"#############################################"<<endl;
	...
	cin>>startSymbol;
	cout<<"Enter File Loctaion: " ;
    
	gmr.addStartSymbol('S');
	gmr.addProduction('S', "ACB|Cbb|Ba");
	gmr.addProduction('A', "da|BC");
	gmr.addProduction('B', "g|@");
	gmr.addProduction('C', "h|@");
	gmr.ComputeFirst();
	gmr.printFirst();
	gmr.ComputeFollow();
	gmr.printFollow();
